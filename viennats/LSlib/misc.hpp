...
//   std::cout << "GetStatusFromDistance(" << value << ")"
//             << "x:" << x << "c1: "
//             << ((value >= 0.)
//                     ? static_cast<value_type>(x) + static_cast<value_type>(0.5)
//                     : static_cast<value_type>(x) - static_cast<value_type>(0.5))
//             << std::endl;
  if (value >= 0.) {
...
//   using lvlset::sign_type::POS_SIGN;
        ...
        if (it.neighbor(i).is_defined()) {
          ...
          if (it.neighbor(i).sign() == 0) { // TODO POS_SIGN
            if (it.center().value() < value_type(-0.5)) {
              oss << "The defined point " << it.center().start_indices()
                  << " has a level set value " << it.center().value() << " but has an "
                     "undefined positive neighbor at "
                     << it.neighbor(i).start_indices()<< "!" << std::endl;
            ...
            if (it.center().value() > value_type(0.5)) {
              oss << "The defined point " << it.center().start_indices()
                  << " has a level set value " << it.center().value() << " but has an "
                     "undefined negative neighbor at "
                     << it.neighbor(i).start_indices() << "!" << std::endl;
