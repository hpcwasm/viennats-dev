...
{
  // std::cout <<"DDD percent=" << percent << std::endl;
  // // std::cout <<"DDD myperc=" << myperc << std::endl;
  ...
  // std::cout <<"inside AddPoint()=" << ",currentList=" <<  currentList<<  ",currentPoint=" << currentPoint << "," <<  pt0 << "," << "," << pt1 << "," << percent << std::endl;
...
void vtkTableBasedClipperVolumeFromVolume::
     ConstructDataSet( vtkDataSet * input,
                       ...
                       TableBasedClipperCommonPointsStructure & cps )
{
  // std::cout <<"vtkTableBasedClipperVolumeFromVolume" << std::endl;
  int   i, j, k, l;
  ...
  // std::cout <<"numUsed=" << numUsed << std::endl; 
  ...
  for ( i = 0; i < nLists; i ++ )
  {
    ...
    for ( j = 0; j < nPts; j ++ )
    {
      ...
      // PAUL proceed on monday here: calculated point coordinates 'pt' are not the same on Ios and desktop!
      // std::cout << pe.percent << "=pe.percent,"  <<  bp << "=bp," <<  p << "=p " << "SetPoint=" << pt[0] << ","<< pt[1]  << "," << pt[2]  << std::endl; 
      outPD->InterpolateEdge( inPD, ptIdx, pe.ptIds[0], pe.ptIds[1], bp );
      // std::cout <<"InterpolateEdge=" << ptIdx << ","<< ptIdx << "," << bp << ","<< pe.ptIds[0] << "," << pe.ptIds[1]  << std::endl; 
      if ( newOrigNodes )
      {
  ...
  for ( i = 0; i < nLists; i ++ )
  {
    ...
    for ( j = 0; j < nPts; j ++ )
    {
      ...
      // std::cout <<"InterpolatePoint=" << ptIdx << std::endl;
      if ( newOrigNodes )
      {
        // std::cout <<"newOrigNodes="<<newOrigNodes << std::endl; 
  ...
  // std::cout <<"#2 ptIdx=" << ptIdx << std::endl; 
    ...
    vtkInformationVector ** inputVector, vtkInformationVector * outputVector )
{
  ...
  {
    // std::cout <<"1# clipAray" <<std::endl;
    // clipAray->PrintSelf(std::cout, vtkIndent(4));
    // cpyInput->PrintSelf(std::cout, vtkIndent(4));
...
void vtkTableBasedClipDataSet::ClipPolyData( vtkDataSet * inputGrd,
     vtkDataArray * clipAray, double isoValue, vtkUnstructuredGrid * outputUG )
{
  // std::cout <<"ClipRectilinearGridData=" << isoValue << std::endl;
...
void vtkTableBasedClipDataSet::ClipRectilinearGridData( vtkDataSet * inputGrd,
     vtkDataArray * clipAray, double isoValue, vtkUnstructuredGrid * outputUG )
{
  // std::cout <<"inside ClipRectilinearGridData=" << isoValue << std::endl;
  ...
  // std::cout <<"isTwoDim=" << isTwoDim << std::endl;
  // std::cout <<"twoDimType=" << twoDimType << std::endl;
  ...
  // std::cout <<"pyStride=" << pyStride << std::endl;
  // std::cout <<"pzStride=" << pzStride << std::endl;
  // std::cout <<"cyStride=" << cyStride << std::endl;
  // std::cout <<"czStride=" << czStride << std::endl;  
  // std::cout <<"cellDims=" << cellDims[0] << "," << cellDims[1] << "," << cellDims[2]  << std::endl;
  // std::cout <<"rectDims=" << rectDims[0] << "," << rectDims[1] << "," << rectDims[2]  << std::endl;    
      ...
      for ( int p = 0; p < nCellPts; p ++ )
      {
        ...
        else if ( pntIndex >= EA && pntIndex <= EL )
        {
          ...
          */
          ...
          // std::cout <<"before AddPoint, p1Weight="<< p1Weight  << std::endl; 
          shapeIds[p] = visItVFV->AddPoint( pntIndx1, pntIndx2, p1Weight );
          // std::cout <<"after AddPoint=" << pntIndx1 << ","<< pntIndx2 << "," << p1Weight  << std::endl; 
          //DDDD
        }
  ...
  // std::cout <<"ConstructDataSet=" << std::endl;  
...
void vtkTableBasedClipDataSet::ClipStructuredGridData( vtkDataSet * inputGrd,
     vtkDataArray * clipAray, double isoValue, vtkUnstructuredGrid * outputUG )
{
  // std::cout <<"ClipStructuredGridData=" << isoValue << std::endl;
...
void vtkTableBasedClipDataSet::ClipUnstructuredGridData( vtkDataSet * inputGrd,
     vtkDataArray * clipAray, double isoValue, vtkUnstructuredGrid * outputUG )
{
  // std::cout <<"ClipUnstructuredGridData=" << isoValue << std::endl;
  ...
  for ( i = 0; i < numCells; i ++ )
  {
    ...
    if ( bCanClip )
    {
      ...
      for ( j = 0; j < nOutputs; j ++ )
      {
        ...
        // std::cout <<"theShape=" << theShape << std::endl;
        switch ( theShape )
        {
          case ST_HEX:
            visItVFV->AddHex( i, shapeIds[0], shapeIds[1],
    ...
    else if (cellType == VTK_POLYHEDRON)
    {
      ...
      numCants ++;
      // std::cout <<"VTK_POLYHEDRON ++numCants=" << numCants << std::endl;
    }
    else
    {
      if ( numCants == 0 )
      ...
      specials->GetCellData()
              ->CopyData( unstruct->GetCellData(), i, numCants );
      numCants ++;
      // std::cout <<"++numCants=" << numCants << std::endl;
    }
